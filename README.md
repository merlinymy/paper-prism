...
- **Scale.** ChatGPT's context window fits ~5-10 papers. PaperPrism indexes hundreds permanently — 1,358 searchable chunks across 20 papers in the demo, always available, never re-uploaded.
...
- **Query adaptation.** ChatGPT processes every question the same way. PaperPrism classifies your question into one of 8 types and changes how it searches — different chunk types, result counts, section filters, and diversity limits per query type.
...
## What Makes It Different From Other RAG Systems?
...
Every question gets classified, and the classification drives the entire retrieval strategy:
...
A factual question searches small, precise chunks and data tables. A comparative question pulls abstracts from many papers but limits results per paper for diversity. No other open-source RAG system dynamically adjusts chunk types, top-k, section filters, and per-paper limits based on what you're asking.

### 6 Chunk Types Per Paper

Instead of splitting papers into uniform chunks, PaperPrism creates 6 representations:

- **Abstract** — paper overview (300 tokens)
- **Section** — logical sections: Methods, Results, Discussion (2000-3000 tokens)
- **Fine** — precise paragraphs with sentence boundaries (500 tokens, 128 overlap)
- **Table** — extracted data tables (IC50 values, assay results)
- **Caption** — figure and table captions
- **Full** — mean-pooled embedding of the entire paper

### Citation Verification

Every `[Source N]` citation in the answer is semantically verified against the actual source passage. Citations are color-coded by confidence: green (verified), amber (partially supported), red (weak support). No other open-source RAG system does this.

### Benchmark Results

Tested against direct Claude (same model, no RAG) on 15 queries with blind, randomized evaluation and reference passages for fact-checking:
